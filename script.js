    ...
    (function(){
        ...
        function setup(container){
            ...
            function loadIframe(){
                ...
                if (!url) return;
                // If image is already loaded (src set in HTML), mark ready immediately
                if (isImage && iframe.complete && iframe.naturalWidth > 0){
                    state.loaded = true;
                    nativeW = iframe.naturalWidth;
                    nativeH = iframe.naturalHeight;
                    iframe.classList.add('is-loaded');
                    container.classList.add('is-ready');
                    applyLayout();
                    return;
                }
                if (iframe.src && !isImage) return;
